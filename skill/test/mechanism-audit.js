...
(async () => {
  ...
  try {
    ...
    const overview = await call('zhigui_get_overview');
    ...
    const stateAfterCheck = await call('zhigui_get_state', { sections: ['currentGoals'] });
    ...
    const textOnlyImpact = await call('zhigui_check_impact', {
    ...
    const aiAssessedImpact = await call('zhigui_check_impact', {
    ...
    const schedule = await call('zhigui_auto_schedule', { startDate: today, days: 1 });
